...
class ArticleModel extends CommonModel {
    ...
    public function index($condition) {
        ...
        foreach ($list['info'] as $k => $v) {
            $list['info'][$k]['title'] = '<a href="/article-'.$list['info'][$k]['cat_id'].'-'.$list['info'][$k]['id'].'.html" target="_blank">'.$list['info'][$k]['title'].'</a>';
            $list['info'][$k]['cUid'] = $this->getUserFullNameByUid($v['cUid']);
            $list['info'][$k]['cat_id'] = $categoryInfo[$v['cat_id']]['name'];
    ...
    public function getCidById($id){
        return M('Article')->getFieldById($id, 'cat_id');
    }
    ...
    public function getListbyAll($order = 'updated_at DESC', $condition) {
        ...
        foreach ($articleList["info"] as $k => $v) {
            ...
            $channelDetail = ChannelModel::getChannelDetailById($v["cat_id"]);
    ...
    public function getDetailById($id) {
        ...
        $channelDetail = ChannelModel::getChannelDetailById($articleList["cat_id"]);
    ...
    public function getListByCode() {
        ...
        $articleMap['cat_id'] = $channelDetail["id"];
        ...
        foreach ($articleList["info"] as $key => $value) {
            ...
            $channelDetail = ChannelModel::getChannelDetailById($value["cat_id"]);
            $articleList["info"][$key]["channelName"] = $channelDetail['name'];
        }
        return $articleList;
    }

    public function getListByCid($cat_id) {

        $channelMap['cat_id'] = $articleMap['cat_id'] = $cat_id;
        $channelDetail = parent::getDetail($param = array('modelName' => 'Channel'), $channelMap);
        $articleList = parent::getPageList($param = array('modelName' => 'Article', 'field' => '*', 'order' => 'updated_at DESC', 'listRows' => '10'), $articleMap);
        foreach ($articleList["info"] as $key => $value) {
            if ($value["thumbnail"] == "") {
    ...
    public function indexFront($firstRow = 0, $listRows = 20) {
        ...
        foreach ($list['info'] as $k => $v) {
            ...
            $list['info'][$k]['cidName'] = $cids[$v['cat_id']]['name'];
        }
        return $list;
    }
    ...
    public function category() {
        if (IS_POST) {
            ...
            $cat = new \Com\Category('Category', array('cat_id', 'pid', 'name', 'fullname'));
     ...
     * @return type
     */
    public function getArticleListPageByCid($cat_id, $firstRow = 0, $listRows = 20) {
        $Article = M('Article');
        $articleMap["cat_id"] = $cat_id;
    ...
    public function getArticleListByCid($cat_id, $limit = 5) {
        $Article = M('Article');
        $articleMap["cat_id"] = $cat_id;
