...
//        $this->checkToken();
        ...
        $time = time() - $time;
        $this->ajaxReturn(array("status" => 1, "info" => "成功备份所选数据库表结构和数据，本次备份共生成了" . ($file_n - 1) . "个SQL文件。耗时：{$time} 秒", "url" => U('SysData/restore')));
    }
...
//检测一行字符串最后有个字符是否是分号，是分号则一条sql语句结束，否则sql还有一部分在下一行中
                if ($tem[$end] == ";") {
                    ...
                    if ($execute > 500) {
                        ...
                        $this->ajaxReturn(array("status" => 1, "info" => '如果导入SQL文件卷较大(多)导入时间可能需要几分钟甚至更久，请耐心等待导入完成，导入期间请勿刷新本页，当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', "url" => U('SysData/restoreData', array(randCode() => randCode()))));
                        exit;
                    }
                } else {
                    $sql.=$tem;
        ...
        $this->ajaxReturn(array("status" => 1, "info" => "导入成功，耗时：{$time} 秒钟"));
    }
    ...
    public function delSqlFiles() {
        if (IS_POST) {
            ...
            if (empty($_POST['sqlFiles']) || count($_POST['sqlFiles']) == 0)
                $this->ajaxReturn(array("status" => 0, "info" => "请先选择要删除的文件"));

            $files = $_POST['sqlFiles'];
            foreach ($files as $file) {
                delDirAndFile(DatabaseBackDir . $file);
            }
            $this->ajaxReturn(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" => __URL__ . "/restore?" . time()));
        }
    }
...
//发送邮件
                    ...
                    $this->ajaxReturn(array("status" => 1, "info" => "如果要发送SQL文件卷较大(多)发送时间可能需要几分钟甚至更久，请耐心等待，发送期间请勿刷新本页。SQL打包成{$sum}个zip包，分{$sum}封邮件发出，<font color=\"red\">当前已经发送完第{$k}封邮件</font>", "url" => U('SysData/sendSql', array(randCode() => randCode()))));
    ...
    function unzipSqlfile() {
        ...
        foreach ($files as $k => $file) {
            ...
            if (count($files) > 1) {
                $this->ajaxReturn(array("status" => 1, "info" => "正在解压缩请耐心等待，解压期间请勿刷新本页 <font color=\"red\">当前已经解压完{$file}</font>", "url" => U('SysData/unzipSqlfile', array(randCode() => randCode()))));
    ...
    public function delZipFiles() {
        if (IS_POST) {
            ...
            $this->ajaxReturn(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" => __URL__ . "/zipList?" . time()));
        }
    }
